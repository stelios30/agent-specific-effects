...
# Implemented here is a new multi-agent version of the sepsis simulator, involving an AI and a clinician agents.
...
from ase.actors.sepsis import SepsisActor, SepsisAction
from ase.tools.utils import get_probability_ranges
from ase.tools.noise import NoiseModel, NullNoiseModel, SepsisStateUniformNoiseModel
...
class SepsisTrajectory(Trajectory):
    ...
    def failed(self) -> bool:
        return self.states[-1].patient_dead
...
class Sepsis(CausalEnv):
    def __init__(
        ...
        if self.dynamics and env_noise_model.order is not None:
            ...
            self.dynamics["initial_state_distribution"] = np.array(insd_ordered)

    def reset(self, rng: Optional[Generator] = None) -> State:
        ...
        if self.dynamics:
            states = range(State.NUM_TOTAL)
            probs = self.dynamics["initial_state_distribution"]
            state = State.from_index(rng.choice(states, p=probs))
    ...
    def step(self, state: Union[State, int], actions: Tuple[int, ...], env_noise: Optional[Dict[str, NoiseModel]] = None, rng: Optional[Generator] = None) -> State:
        # if rng is not specified, create a new one with random seed
        rng = rng or np.random.default_rng()
        ...
        if not self.turn_based:
            ...
            return replace(state, player=1 - state.player, act_ai=actions[state.player])
        elif state.player == 1:
    ...
    def sample_trajectory(
        ...
        t = SepsisTrajectory(id=t.id, states=t.states, actions=t.actions, probs=t.probs, env_noise=t.env_noise, act_noise=t.act_noise)
        return t
    ...
    def _get_env_noise(self, trajectory: Optional[Trajectory] = None, time_step: Optional[int] = None, rng: Optional[Generator] = None) -> Dict[str, NoiseModel]:
        ...
        state = trajectory.states[time_step]
        state_next = trajectory.states[time_step + 1]

        # do not sample noise for steps when AI agent acts as state
        # transitions deterministically in this case
        ...
        action = trajectory.actions[time_step][state.player]
