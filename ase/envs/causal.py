...
class Trajectory:
    ...
    def reward(self, discount: float = 0.99) -> float:
        discounts = discount ** np.arange(self.horizon, dtype=np.float32)
        rewards = np.array([s.reward for s in self.states])
...
class CausalEnv:
    ...
    def sample_trajectory(
        ...
        state = copy.deepcopy(initial_state) if initial_state is not None else self.reset(rng=rng)
        ...
        for t in range(horizon - 1):
            ...
            if env_noise and t < len(env_noise) and env_noise[t] is not None:
                # do not generate environment noise, if it is explicitly passed
                curr_env_noise = env_noise[t]
            else:
                # sample noise for the current state
                curr_env_noise = self._get_env_noise(rng=rng)
            ...
            state = self.step(state, actions, curr_env_noise, rng)
    ...
    def sample_noise_from_posterior(self, trajectory: Trajectory, agents: List[CausalActor], rng: Optional[Generator] = None) -> Tuple[List[List[NoiseModel]], List[Dict[str, NoiseModel]]]:
        ...
        for t in range(trajectory.horizon - 1):
